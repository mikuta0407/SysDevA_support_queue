...
## PHP Development Serverを用いた開発環境構築について

作者自身の開発環境は、PHPのCLI SAPIにビルトインされているPHP Development Serverを使用しました。

```bash
php -S localhost:3000
```

(ポート番号及びアドレスについては環境に応じて変更する必要があります)

## Dockerを用いた開発環境構築について

プロジェクトのルートディレクトリにて、以下のコマンドで起動することにより、Dockerを用いて開発することができます。
